class HassConnector:
    ...
    def addAll(self, referenceList):
        ...
        for r in referenceList:
            if "r" in r.rw and "w" not in r.rw:
            ...
            if "w" in r.rw and "r" in r.rw:
                if r.poller.dataType == "bool":
                    self.addSwitch(r)
                # currently I have no idea what entity type to use here ...
                if r.poller.dataType == "int16":
                    self.addSensor(r)
    ...
    def addBinarySensor(self, ref):
        ...
        self.mqc.publish("homeassistant/binary_sensor/" +
                         self.globaltopic[0:-1] + "_" + ref.device.name +
                         "_" + ref.topic + "/config",
                         "{\"name\": \"" + ref.device.name + "_" + ref.topic +
                         "\", \"state_topic\": \"" + self.globaltopic +
                         ref.device.name + "/state/" + ref.topic +
                         "\", \"payload_on\": \"True\", \"payload_off\": \
                         \"False\"}",
                         qos=0,
                         retain=True)
    ...
    def addSensor(self, ref):
        ...
        self.mqc.publish("homeassistant/sensor/" + self.globaltopic[0:-1] +
                         "_" + ref.device.name + "_" + ref.topic + "/config",
                         "{\"name\": \"" + ref.device.name + "_" + ref.topic +
                         "\", \"state_topic\": \"" + self.globaltopic +
                         ref.device.name + "/state/" + ref.topic+"\"}",
                         qos=0,
    ...
    def addSwitch(self, ref):
        ...
        self.mqc.publish("homeassistant/switch/" + self.globaltopic[0:-1] +
                         "_" + ref.device.name + "_" + ref.topic + "/config",
                         "{\"name\": \"" + ref.device.name + "_" + ref.topic +
                         "\", \"state_topic\": \"" + self.globaltopic +
                         ref.device.name + "/state/" + ref.topic +
                         "\", \"state_on\": \"True\", \"state_off\": \"False\", \
                         \"command_topic\": \"" + self.globaltopic +
                         ref.device.name + "/set/" + ref.topic +
                         "\", \"payload_on\": \"True\", \"payload_off\": \
                         \"False\"}",
                         qos=0,
                         retain=True)
